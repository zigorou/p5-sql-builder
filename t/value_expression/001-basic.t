...
subtest "_value_expression() with array_ref" => sub {
    my $sql = SQL::Builder->new;
    my ( $stmt, @bind );
    ( $stmt, @bind ) = $sql->_value_expression(
        [ 'UNIX_TIMESTAMP(?) - created_on AS duration', '2010-01-31 00:00:00' ]
    );
    is( $stmt, 'UNIX_TIMESTAMP(?) - created_on AS duration', 'array_ref stmt' );
    is_deeply( \@bind, ['2010-01-31 00:00:00'], 'array_ref bind' );
    done_testing;
};
...
subtest "_value_expression() with hash_ref" => sub {
...

    {
        ...
        is(
            ...
            'hash_ref stmt with as cause, array_ref value, upper_case option'
        );
        is_deeply( \@bind, ['2010-01-31 00:00:00'],
            'hash_ref bind with as cause, array_ref bind, upper_case option' );
    }

    {
        ...
        is(
            $stmt,
            'UNIX_TIMESTAMP(?) - created_on as duration',
            'hash_ref stmt with as cause, array_ref value, lower case option'
        );
        is_deeply( \@bind, ['2010-01-31 00:00:00'],
            'hash_ref bind with as cause, array_ref bind, lower case option' );
    }

    {
        ...
        is(
            ...
            'hash_ref stmt with as cause, array_ref bind, with_paren'
        );
        is_deeply( \@bind, ['female', 2],
            'hash_ref bind with as cause, array_ref bind, with_parent' );
    }
